...
function conquistas(perfil: PerfilQuest): Conquista[] {
  ...
  return [
    ...
    // Conquistas de gameplay derivadas do estado real do perfil (o loop
    // jogável credita 1 estrela na 1ª conclusão de missão e mantém a sequência
    // de dias). "Explorador de planetas" fica pendente: não há contagem de
    // mundos visitados no perfil — depende de um dado que o backend ainda não
    // expõe, então permanece bloqueada em vez de exibir um progresso inventado.
    { icone: "🎯", nome: "Primeira missão", descricao: "Complete sua primeira missão", obtida: perfil.estrelas_total > 0 },
    { icone: "⭐", nome: "Colecionador de estrelas", descricao: "Junte 100 estrelas", obtida: perfil.estrelas_total >= 100 },
    { icone: "🪐", nome: "Explorador de planetas", descricao: "Visite 3 planetas", obtida: false },
    { icone: "🔥", nome: "Chama de 7 dias", descricao: "Jogue 7 dias seguidos", obtida: perfil.sequencia_dias >= 7 },
  ];
}
...
function PainelAprendizado() {
  ...
  const carregar = useCallback(() => {
    ...
    minhasConquistas()
      ...
      // Erro ≠ "sem conquistas": uma falha de rede não pode virar a mensagem
      // acolhedora de lista vazia (senão a criança "não tem conquistas" quando
      // na verdade o servidor não respondeu). Guarda um estado de erro próprio.
      .catch(() => { if (vivo) { setErro(true); setItens(null); } })
  ...
  if (erro) {
    return (
      <div className="painel">
        <h2>Conquistas de aprendizado</h2>
        <p className="conquistas-carregando">Não consegui carregar suas conquistas agora. 📡</p>
        <button className="conquistas-retry" onClick={() => carregar()}>Tentar de novo</button>
      </div>
    );
  }
  const lista = itens ?? [];
