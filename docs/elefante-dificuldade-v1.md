...
| P1 | `nota_elefante_institucional = 0` com `nota_elefante > 0` (migração 0028 sem backfill; a rede lia zero) | A nota recebe o carimbo `detalhes.regua_institucional = {versao_dificuldade, perfil_local}`; a rede só agrega notas carimbadas e `dashboard_rede` expõe `recalculo_pendente` (bloco operacional, fora dos cartões de métricas); `recalcular_institucional.py --pendentes` recalcula só quem falta (idempotente, nunca grava zero) | `scoring.carimbo_institucional`, `rede._CARIMBO_INSTITUCIONAL`, `rede.escolas_com_notas_pendentes` |
...
conferir `recalculo_pendente.total == 0` em `GET /rede/dashboard` (ou `--pendentes` de novo: 0 escolas) → (opcional) auditoria read-only.
