...
class Rede(Base):
    ...
    que dá escopo ao perfil "Secretaria/Admin de rede": um usuário com
    ``rede_id`` enxerga TODAS as escolas dessa rede (mas não as de outras redes),
    sem precisar ser ``is_global``. O isolamento continua sendo por escola — a
    rede só define QUAIS escolas entram no escopo do usuário.
    """
